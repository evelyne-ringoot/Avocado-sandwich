...
void run_config( Phase phase,
    ...
    double *d_A, *d_S, *d_work;
    int *d_info;
    
    HIP_CHECK(hipMalloc(&d_A, size_in * size_in * sizeof(double)));
    HIP_CHECK(hipMalloc(&d_S, size_in * sizeof(double)));
    HIP_CHECK(hipMalloc(&d_work, size_in * sizeof(double)));
    ...
    double elapsed_ms = benchmark_ms(
        ...
        2,
        [&]() {
            ROCRAND_CHECK(rocrand_generate_uniform_double(gen, d_A, size_in*size_in));
        },
        [&]() {
