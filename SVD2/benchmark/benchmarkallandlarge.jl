...
for (i,size_i) in enumerate(sizes)
    timings[i] = min( benchmark_ms(size_i,functobench), timings[i])
end
...
writedlm(string(functobench)*"results"*string(elty)*"_"* string(TILESIZE)* "_"* string(TILESIZEMUL)* "_"* string(QRSPLIT)* "_small.csv",  output, ',')
...
output[:,5].= sizes
    ...
    for (i,size_i) in enumerate(sizes)
        timing = benchmark_ms_large(size_i,functobench)
        @printf " %4d   %8.02e    %8.02f   \n" size_i 0.0 timing
        output[i,6]=round.(Int,timing*10)
        writedlm(string(functobench)* "results"*string(elty)*"_"* string(TILESIZE)* "_"* string(TILESIZEMUL)* "_"* string(QRSPLIT)* "_large.csv",  output, ',')
    end
